...
# 7-Zs 8-SFR 9-J 10-V 11-Rstar 12-Rtot 13-Rvmax (14-U 15-K 16-g; not in SIMBA)

# find the elements not discarded already
elements = np.arange(14)
elements = np.delete(elements, discarded)

# get the beginning of the suffix
suffix = ''
for i in discarded:
    suffix += '_%d' % i

# do a loop over all properties
for element in discard:

    print('Removing property %d' % element)
    ...
    index   = np.where(elements==element)[0]
    indexes = np.delete(elements, index)
    ...
    storage = 'sqlite:///databases/TPE_%s%s_%d.db' % (sim, suffix, element)
    fout = 'losses/loss_XGB_Om_%s%s_%d.txt' % (sim, suffix, element)
