...
class Objective(object):
    ...
    def __call__(self, trial):
        ...
        L1           = trial.suggest_float('L1', 0, 10)
        L2           = trial.suggest_float('L2', 0, 10)
...
def get_dataset(f_input, f_off, f_output, seed, mode):
    ...
    prop = np.zeros((Ngal, data.shape[1]+1), dtype=np.float32)
    ...
    for i in indexes:
        for j in range(length[i]):
            prop[count,:-1] = data[off[i] + j]
            prop[count,-1]  = params[i,0]
            pars[count]     = params[i]
            count += 1
...
suffix = 'all+Om_ASN1'
storage = 'sqlite:///databases/TPE_%s_%s.db'%(sim,suffix)
fout = 'losses/loss_XGB_Om_%s_%s.txt'%(sim,suffix)
####################################################################################
